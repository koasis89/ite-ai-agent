...
export interface ASTNode {
  type: ASTNodeType;
  value?: string;           // text, code, inlineCode, html 등 단일 리프 노드용
  depth?: number;           // heading 노드의 깊이 (1~6)
  ordered?: boolean;        // list 노드 정렬 상태 여부
  lang?: string;            // code 노드 프로그래밍 언어
  children?: ASTNode[];     // 자식 컨테이너 노드용
}
 ...
 */
export function parseMarkdownToCustomAST(markdown: string): MarkdownRoot {
  const root: MarkdownRoot = { type: "root", children: [] };
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("|")) {
      ...
      if (nextLine.startsWith("|") && isSeparatorRow(nextLine)) {
        const tableNode: ASTNode = { type: "table", children: [] };

        // 헤더 로우 파싱
        const headerCells = splitTableRow(line);
        const headerRowNode: ASTNode = {
          type: "tableRow",
...
// ─── 3. 컬럼 매칭 파이프라인 (Column Matching Pipeline) ──────────────────────────

export interface ManifestColumn {
  idx: number;                         // Excel/Word target index (1-based)
  field: string;                       // Field identifier (e.g., "wbsId", "task", "owner")
  type: "string" | "number" | "boolean"; // Target datatype
}
 ...
 */
export function findMatchingField(headerName: string): string | null {
 ...
 */
export function castValueType(value: string, type: "string" | "number" | "boolean"): string | number | boolean {
 ...
 */
export function normalizeTableToJSON(
  tableNode: ASTNode,
  requiredColumns: ManifestColumn[],
  ...
  const codeIndexToManifestField = new Map<number, ManifestColumn>();
