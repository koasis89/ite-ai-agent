...
- **체크리스트**:
  - [x] 매니페스트 인자를 기반으로 서브 프로세스 그룹을 동적 구동하는 매니저 구조 확립
  - [x] **[자원 누수 게이트]** Main Process 종료 시 자식 MCP 프로세스를 영구 파괴하는 Teardown 훅 탑재
  - [x] `stdio` 스트림 파이프 바인딩 및 입출력 채널 정상 격리 확인
  - [x] `mcp-bridge.test.ts` 에서 가상 MCP stdio 에코 테스트 세션 통과
...
- **체크리스트**:
  - [x] 파일 워처 인터페이스 기반 신규 파일 생성 및 리스너 가동 구현
  - [x] **[파싱 안전 게이트]** incomplete 스트림 청크를 안전하게 조립하는 라인 버퍼 어셈블러 구현 완료
  - [x] 날짜 변경 및 로테이션 발생 시 파일 포인터 무중단 추적 전환 유효성 확인
  - [x] `hook-tailer.test.ts` 에서 모킹된 고속 JSONL 데이터 쓰기 테스트 시 누수 0% 달성
...
- **체크리스트**:
  - [x] `hooks-extension-ko.md` 양식 계약에 부합하는 Zod 이벤트 필터 가드 구현
  - [x] UI 바인딩용 `needs-input` 및 도구 실행 전후 상태 알림용 이벤트 특화 디스패처 완료
  - [x] Renderer 레이어로 이벤트를 단방향 송신할 `omx:runtime-hook-event` IPC 채널 등록
  - [x] `event-dispatcher.test.ts` 단위 테스트 작성 및 IPC 이벤트 전송 성공 확인
